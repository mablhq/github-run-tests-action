...
export function optionalArrayInput(name) {
    return core
        .getInput(name, {
        required: false,
    })
...
export function booleanInput(name, defaultValue = false) {
...
export async function run(enableFailureExitCodes = true) {
    ...
    try {
        ...
        if (uri) {
            core.warning(`[${ActionInputs.Uri}] has been deprecated. Please use [${ActionInputs.UrlApp}] instead.`);
        }
        if (uri && appUrl) {
        ...
        const apiClient = new MablApiClient(apiKey);
        ...
        if (!awaitCompletion) {
            core.info('Test plan(s) triggered. Not awaiting completion because await-completion=false. See mabl app or APIs for final results.');
            core.endGroup();
            return;
        }
        ...
        core.setOutput(ActionOutputs.PlansRun, '' + finalExecutionResult.plan_execution_metrics.total);
        core.setOutput(ActionOutputs.PlansPassed, '' + finalExecutionResult.plan_execution_metrics.passed);
        core.setOutput(ActionOutputs.PlansFailed, '' + finalExecutionResult.plan_execution_metrics.failed);
        core.setOutput(ActionOutputs.TestsRun, '' + finalExecutionResult.journey_execution_metrics.total);
        core.setOutput(ActionOutputs.TestsPassed, '' + finalExecutionResult.journey_execution_metrics.passed);
        core.setOutput(ActionOutputs.TestsFailed, '' + finalExecutionResult.journey_execution_metrics.failed);
